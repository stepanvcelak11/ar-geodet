...
// PWA: nova verze ceka -> nabidnout obnovu (resi matouci starou cache)
        ...
        // Verze = cislo SHELL_CACHE (razitko ?v= u style.css, doplnuje vydani). Do 12. 9. 2026
        // tu byla natvrdo '1.9', ktera se nikdy nebumpla — O aplikaci hlasilo 1.9, Historie
        // aktualizaci vedle toho v299.
        function appVersion() {
            try {
        ...
        function handleCameraError(err) {
            ...
            let msg;
            // NÁVOD PODLE TOHO, ODKUD APPKA BĚŽÍ (15. 9. 2026): z Google Play (TWA) nemá appka
            // adresní řádek ani vlastní položku „Kamera" v Nastavení telefonu (tam je jen Poloha,
            // kterou si Play appka bere sama). Kameru drží CHROME: musí ji mít povolenou od
            // systému a pro web appky v Nastavení webu. Kamarád uživatele s Androidem tak
            // podle staré hlášky („ikona zámku v adresním řádku") neměl kam sáhnout.
            const _twa = (function () { try { return !!(window.AGProZamky && AGProZamky.jeTwa && AGProZamky.jeTwa()) || (document.referrer || '').indexOf('android-app://') === 0 || localStorage.getItem('agTwa_v1') === '1'; } catch (e) { return false; } })();
            const _ios = /iPhone|iPad/.test(navigator.userAgent);
            const _android = /Android/.test(navigator.userAgent);
            let navod;
            if (_ios) navod = '• iPhone: Nastavení → <b>QTRIG</b> (appka z plochy) nebo <b>Safari</b> → Kamera → Povolit. Pak appku zavři a otevři znovu.';
            else if (_twa || _android) navod = (_twa ? '• Appka z Google Play běží v Chromu, kamera se povoluje V CHROMU (v Nastavení telefonu má QTRIG jen Polohu — to je v pořádku).<br>' : '')
                + '• Nastavení telefonu → Aplikace → <b>Chrome</b> → Oprávnění → <b>Kamera</b> → Povolit.<br>'
                + '• Chrome → ⋮ → Nastavení → Nastavení webu → <b>Kamera</b> → v seznamu <b>Blokováno</b> najdi <b>stepanvcelak11.github.io</b> → Povolit.<br>'
                + '• Pak QTRIG úplně zavři a otevři znovu.';
            else navod = '• V prohlížeči: ikona zámku v adresním řádku → Oprávnění → Kamera → Povolit, pak stránku obnov.';
            if (denied) msg = 'Aplikace nemá povolený přístup ke kameře, takže AR nejde spustit.<br><br><b>Jak kameru povolit:</b><br>' + navod + '<br><br>Zatím je zapnutý režim <b>Mapa</b> — vše kromě AR funguje dál.';
        ...
        function renderNearbyList() {
            ...
            if (!pts.length) { var _mimoCR = false; try { _mimoCR = !!(window.AGSour && !AGSour.jeCZ()); } catch (e) { _mimoCR = false; } listDiv.innerHTML = '<p style="text-align:center; opacity:0.7;">' + (_mimoCR ? 'Žádné body v dosahu. Úřední body se stahují jen tam, kde je stát zveřejňuje jako data: Česko (ČÚZK), Slovensko (GKÚ SR), Švýcarsko (swisstopo), Nizozemsko (Kadaster), Francie (IGN), Španělsko (IGN), Rakousko (BEV), v Německu Berlín, Meklenbursko-Přední Pomořansko a Bádensko-Württembersko (AFIS). Jinde jsou v mapě jen vlastní body (Nový bod, import, výkres).' : 'Žádné body v dosahu.') + '</p>'; return; }
            pts.forEach(({ pt, d }) => {
                let typBodu = "Podrobný polohový bod"; if (pt.cat === 'TB') typBodu = "Trigonometrický bod"; if (pt.cat === 'ZHB') typBodu = "Zhušťovací bod"; if (pt.cat === 'NIVEL') typBodu = "Nivelační / Výškový bod"; if (pt.cat === 'TIHA') typBodu = "Tíhový bod"; if (pt.cat === 'CUSTOM') typBodu = "Vlastní bod";
                if (agZHodinek(pt)) typBodu = "Bod z hodinek";
                let col = agBarvaBodu(pt);
        ...
        function setModeSeg(m) {
            ...
            var sd = document.getElementById('seg-des'), dd = String(window.agDes ? agDes() : 2);
            if (sd) sd.querySelectorAll('.st-seg-b').forEach(function (b) { b.classList.toggle('on', b.getAttribute('data-des') === dd); });
        }
        function syncSettingsControls() {
            try { var sd0 = document.getElementById('seg-des'), dd0 = String(window.agDes ? agDes() : 2); if (sd0) sd0.querySelectorAll('.st-seg-b').forEach(function (b) { b.classList.toggle('on', b.getAttribute('data-des') === dd0); }); } catch (e) { /* nic */ }
        ...
        function _mngYX(pt) {
            ...
            if (!c || c.k !== k) {
                ...
                c = { k: k, y: agFmtM(s.y), x: agFmtM(s.x) };
        ...
        function _mngBuildDetail(pt) {
            ...
            det.innerHTML = '<div class="mngr-grid">'
                ...
                + '<div><i>Z</i><b>' + (pt.vyska != null ? agFmtM(pt.vyska) + ' m' : '—') + '</b></div>'
        ...
        function _mngCopy(pt) {
            const yx = _mngYX(pt);
            const txt = pt.name + ';' + yx.y + ';' + yx.x + (pt.vyska != null ? ';' + agFmtM(pt.vyska) : '');
        ...
        function renderLinesList(listDiv) {
            ...
            pointLines.forEach(ln => {
                ...
                item.innerHTML = `<div class="cp-title" style="color:var(--warning,#fbbf24);">#${an} \u2194 #${bn}</div><div class="cp-coords">Délka: ${d.toFixed(1).replace('.', ',')} m</div><div class="cp-actions"><button class="cp-btn cp-btn-delete" onclick="deleteLineFromList('${ln.id}')"><svg class="icon"><use href="#i-trash"/></svg></button></div>`;
        ...
        function renderAR(event) {
            ...
            const _cdTitle = !headingReliable ? 'Zařízení neposkytuje absolutní azimut – sever může být nepřesný. Dolaď v Nastavení kompasu „Srovnání severu".' : (calWarn ? 'Kompas vyžaduje kalibraci – udělej telefonem osmičku' : '');
        ...
        function addDictEntry() {
            ...
            if (!t || !d) { agInfo('Vyplň pojem i vysvětlení.'); return; }
