// ===== QTRIG — KARTA BODU, obsah (ODLOŽENÝ modul; eager shim je js/karta-bodu.js) =====
...
(function () {
    'use strict';
    if (window.AGKartaBoduPlus) return;
    ...
    function injectStyles() {
        ...
        st.textContent = [
            ...
            // světlý motiv: žlutá a zelená na bílé nejsou čitelné
            'body.light-mode .ag-kb-om b,body.light-mode .ag-kb-t.warn b{color:#b45309;}body.light-mode .ag-kb-t.ok b{color:#1f8c66;}'
        ].join('\n');
    ...
    function init() {
        ...
        if (!window.__agKbWatch) {
            window.__agKbWatch = setInterval(relabel, 1500);
            setTimeout(function () { clearInterval(window.__agKbWatch); window.__agKbWatch = 0; }, 15000);
        }
    }
    init();
    window.AGKartaBoduPlus = { render: render, refresh: function () { if (_pt) render(_pt); } };
    // karta uz muze byt otevrena (shim ji otevrel driv, nez tenhle modul dojel)
    try { var bs = document.getElementById('bottom-sheet'); if (bs && bs.classList.contains('open') && window.__agKbCekaBod) render(window.__agKbCekaBod); } catch (e) { }
})();
