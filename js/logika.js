...
if ('serviceWorker' in navigator) {
        ...
        function deleteCustomPoint(id, skipConfirm) { const _pt = persistentCustomPoints.find(p => p.id === id); if (!skipConfirm) { agGuard('Smazat bod „' + ((_pt && _pt.name) || 'bez názvu') + '"?', function () { deleteCustomPoint(id, true); }, { danger: true }); return; } persistentCustomPoints = persistentCustomPoints.filter(p => p.id !== id); setStoredData('arCustomPoints12', JSON.stringify(persistentCustomPoints)); pointLines = pointLines.filter(l => l.aId !== id && l.bId !== id); saveLines(); renderManageList(); drawAllMarkersOnMap(); const idx = arPoints.findIndex(p => p.id === id); if(idx !== -1) { if(arPoints[idx].element) arPoints[idx].element.remove(); arPoints.splice(idx, 1); } updateInfoPanel(); }
