...
(function () {
    ...
    function styl() {
        ...
        var css = ''
            ...
            + '.ag-maskot .mk-btn{flex:0 0 auto;width:66px;height:80px;padding:0;border:0;background:none;cursor:pointer;-webkit-tap-highlight-color:transparent;}'
            ...
            + '.ag-maskot.mk-off .mk-bublina{display:none;}.ag-maskot.mk-off .mk-btn{width:34px;height:42px;opacity:.75;}'
            ...
            + '.ag-maskot.mk-roh .mk-btn{width:56px;height:68px;filter:drop-shadow(0 4px 10px rgba(0,0,0,.35));}'
    ...
    function vytvor(roh) {
        ...
        el.innerHTML = '<button type="button" class="mk-btn" aria-label="' + esc(t('Maskot') + ' ' + s.jmeno + ' — ' + t('další rada')) + '">' + SVG + '</button>'
            + '<div class="mk-bublina" role="status" aria-live="polite"><span class="mk-jmeno"></span><span class="mk-text"></span>'
        ...
        el.querySelector('.mk-btn').addEventListener('click', function () {
            ...
            rekniDo(el, Math.random() < 0.35 ? 'klepnuti' : 'tip', null, Math.random() < 0.5 ? 'radost' : 'mysli');
        });
    ...
    function rekniDo(el, kat, data, nalada) {
        if (!el || !nast().zap) return;
        var txt = vyber(kat, data); if (!txt) return;
        ...
        el._mkT2 = setTimeout(function () { el.classList.remove('mk-mluvi', 'mk-smutek'); if (el.classList.contains('mk-roh')) el.classList.add('mk-ticho'); }, Math.max(5000, txt.length * 70));
    }
    ...
    window.AGMaskot = {
        pripoj: pripoj, rekni: rekni, nastaveni: panel,
        _test: { rozparsuj: rozparsuj, vyber: vyber, hlasky: hlasky, vzor: vzor, H: H, KAT: KAT }
    };
})();
