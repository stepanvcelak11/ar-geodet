...
(function () {
    ...
    // Vzácná okna účtu jsou v js/ucty-okna.js (d1, 25. 9. 2026) — načtou se až při prvním otevření.
    var _okna = null, _oknaFronta = null;
    function okna(cb) {
        if (_okna) return cb(_okna);
        if (_oknaFronta) { _oknaFronta.push(cb); return; }
        _oknaFronta = [cb];
        var hotovo = function () {
            var q = _oknaFronta; _oknaFronta = null;
            if (!window.AGUctyOkna) { toast('Okno se nenačetlo — zkontroluj připojení a zkus to znovu.'); return; }
            _okna = window.AGUctyOkna({ DEFAULT_API: DEFAULT_API, LS_ACC: LS_ACC, LS_FIRM: LS_FIRM, LS_IDCUR: LS_IDCUR, LS_LAST: LS_LAST, LS_OFF: LS_OFF, LS_SPACES: LS_SPACES, LS_SYNC: LS_SYNC, LS_TOK: LS_TOK, LS_TRUST: LS_TRUST, adoptLogin: adoptLogin, aktualniProstor: aktualniProstor, applyPerms: applyPerms, brandHtml: brandHtml, bustFirm: bustFirm, cloudFetch: cloudFetch, ensureLib: ensureLib, enterApp: enterApp, esc: esc, failClear: failClear, fillMark: fillMark, getFirm: getFirm, getProstory: getProstory, getTok: getTok, getUcet: getUcet, injectStyles: injectStyles, listProfiles: listProfiles, lsKontakt: lsKontakt, maProstory: maProstory, prepniProstor: prepniProstor, profileKeyOf: profileKeyOf, removeProfile: removeProfile, setProstory: setProstory, setSess: setSess, showGate: showGate, startLive: startLive, switchProfile: switchProfile, terrainHtml: terrainHtml, tick: tick, usageLog: usageLog });
            q.forEach(function (f) { try { f(_okna); } catch (e) { window.AG && AG.swallow && AG.swallow(e, 'ucty:okna'); } });
        };
        var nacti = function () { var s = document.createElement('script'); s.src = 'js/ucty-okna.js'; s.onload = hotovo; s.onerror = hotovo; document.head.appendChild(s); };
        // Po startu ho ag/lazy (js/lazy-load.js) stejně dotáhne v nečinnosti — need() nestahuje podruhé.
        // Na bráně před startem ještě ve frontě není: need() pak zavolá hned a načte se ručně.
        if (window.AGLazy && AGLazy.need) AGLazy.need('js/ucty-okna.js', function () { if (window.AGUctyOkna) hotovo(); else nacti(); });
        else nacti();
    }
    ...
    function showRegister() { var a = arguments; okna(function (O) { O.showRegister.apply(null, a); }); }   // → js/ucty-okna.js

    function showSmazaniUctu() { var a = arguments; okna(function (O) { O.showSmazaniUctu.apply(null, a); }); }   // → js/ucty-okna.js
    ...
    function showProstory() { var a = arguments; okna(function (O) { O.showProstory.apply(null, a); }); }   // → js/ucty-okna.js
    ...
    function showFirmy() { var a = arguments; okna(function (O) { O.showFirmy.apply(null, a); }); }   // → js/ucty-okna.js
    ...
    function ukazKodUctu() { var a = arguments; okna(function (O) { O.ukazKodUctu.apply(null, a); }); }   // → js/ucty-okna.js

    function showObnova() { var a = arguments; okna(function (O) { O.showObnova.apply(null, a); }); }   // → js/ucty-okna.js

    function showNovyObnovovaciKod() { var a = arguments; okna(function (O) { O.showNovyObnovovaciKod.apply(null, a); }); }   // → js/ucty-okna.js

    function scanFirmQR() { var a = arguments; okna(function (O) { O.scanFirmQR.apply(null, a); }); }   // → js/ucty-okna.js
