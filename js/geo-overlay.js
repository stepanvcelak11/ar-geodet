...
(function () {
    ...
    // =====================================================================
    function idb() { return new Promise(function (res, rej) { try { var r = indexedDB.open(DB, 1); r.onupgradeneeded = function () { try { r.result.createObjectStore(STORE); } catch (e) { window.AG && AG.swallow && AG.swallow(e, 'geo-overlay:onupgradeneeded'); } }; r.onsuccess = function () { var d = r.result; if (d && !d.objectStoreNames.contains(STORE)) { /* samoléčba (18. 9. 2026): databázi kdysi založil prázdnou přehled místa (ag-store) — o verzi výš sklad doplní */ var v2 = d.version + 1; try { d.close(); } catch (e) { } var r2 = indexedDB.open(DB, v2); r2.onupgradeneeded = function () { try { r2.result.createObjectStore(STORE); } catch (e) { } }; r2.onsuccess = function () { res(r2.result); }; r2.onerror = function () { rej(r2.error); }; return; } res(d); }; r.onerror = function () { rej(r.error); }; } catch (e) { rej(e); } }); }
