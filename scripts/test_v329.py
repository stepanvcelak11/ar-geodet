...
async def beh(url):
    ...
    async with async_playwright() as pw:
        ...
        # 18. 9. 2026 (úklid nástrojů, v360): Oměrné, Stopa trasy, rozcestník Srovnat jinak a Dnešek v terénu dal uživatel STRANOU (hidden)
        ok('B1 Oměrné v seznamu úkonů NEJSOU (od 18. 9. 2026 schované — uživatel: dal bych pryč)', 'openCheckDist' not in b, list(b.keys())[:30])
        ok('B2 Stopa trasy v seznamu úkonů NENÍ (schovaná 18. 9. 2026)', 'track-log' not in b)
        ...
        ok('B3 řádek „Opravit GPS" vypisuje své položky živě (chůzí po hraně · z mapy · podle bodu), Srovnat jinak je pryč', 'opravit-gps' in b and 'hraně' in b['opravit-gps'] and 'známého bodu' in b['opravit-gps'] and 'srovnat-sever' not in b, b.get('opravit-gps'))
        inhub = await page.evaluate("() => AGReg.hubOf('kalibrace-hranou')")
        ok('B3b kalibrace chůzí po hraně je položka rozcestníku Opravit GPS (ne samostatný řádek)', 'kalibrace-hranou' not in b and inhub == 'opravit-gps', inhub)
        ...
        ok('B5 řádek „Počasí a světlo" vypisuje počasí a slunce, Dnešek v terénu už ne (schovaný 18. 9. 2026)', 'pocasi-svetlo' in b and 'počasí' in b['pocasi-svetlo'] and 'dnešek' not in b['pocasi-svetlo'], b.get('pocasi-svetlo'))
