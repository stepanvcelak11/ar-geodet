...
async def beh(url):
    ...
    async with async_playwright() as pw:
        ...
        await page.evaluate("() => document.getElementById('ag3d-pitch').click()"); await page.wait_for_timeout(700)
        ...
        await page.evaluate("() => document.getElementById('ag3d-teren').click()"); await page.wait_for_timeout(700)
        ...
        j2 = await page.evaluate("""() => { var m = 111320, ml = m * Math.cos(%f * Math.PI / 180); var uv = AGOkoli.vyhodnot(%f - 65 / m, %f + 5 / ml); var u = AGOkoli.vyhodnot(%f - 58 / m, %f + 5 / ml); var n = 0; map.eachLayer(l => { if (l instanceof L.Polygon && l.options.dashArray === '8,5') n++; }); return { uvnitr: uv.prekazkaUvnitr, u: u.prekazka && u.prekazka.d, obdelniky: n, ulozeno: !!getStoredData('agPrekazky') }; }""" % (LAT, LAT, LNG, LAT, LNG))
        ...
        ok('L2 paska miri na dalsi lom (smer != primy smer o > 5°), zbyva = delka trasy', l1 and isinstance(l1.get('smer'), (int, float)) and abs(((l1['smer'] - l1['primySmer'] + 540) % 360) - 180) > 5 and abs(l1['zbyva'] - l1['delka']) < 3.0, l1)
        ...
        # ================= M: panel „Mapa" ve trech patrech (17. 9. 2026, js/mapa-panel.js) =====
        ...
        ok('M2 styl Noc = setStyle vektorove mapy', await cekej(page, "AGMapaVektor.nastaveni().styl === 'noc' && AGMapaVektor.mapa() && AGMapaVektor.mapa().getStyle() && AGMapaVektor.mapa().getStyle().name.endsWith('noc')", 20),
           await page.evaluate("() => { var m = AGMapaVektor.mapa(); return { styl: AGMapaVektor.nastaveni().styl, stav: AGMapaVektor.stav(), mapa: !!m, name: m && m.getStyle() && m.getStyle().name, on: document.querySelector('#ms-styl .on') && document.querySelector('#ms-styl .on').getAttribute('data-styl') }; }"))
        await page.evaluate("() => document.getElementById('btn-baselayer').click()")
        await page.wait_for_timeout(300)
        ...
        # vypnuti jde FRONTOU za zapni() ze zmeny stylu (M2) — na pomalem stroji 800 ms nestacilo (18. 9. 2026)
        await cekej(page, "AGMapaVektor.stav() === 'vypnuto'", 30)
        await page.wait_for_timeout(300)
        ...
        ok('M6 zalozka Vrstvy: prepinac Trasa terenem prepina AGTrasa; radek Prekazky schovany bez prekazek', m6 and m6['vidTrasa'] and m6['pred'] and not m6['po'] and m6['zpet'] and m6['prekazkyHidden'], m6)
        m7 = await page.evaluate("""() => { var m = 1 / 111320; AGOkoli.pridejPrekazku({ lat: %f + 50 * m, lng: %f }, { lat: %f + 56 * m, lng: %f + 8 * m }, 'hromada', 'hromada'); AGMapaPanel.sync(); var rp = document.getElementById('ms-prekazky'); var vid = !rp.hidden && rp.classList.contains('ctrl-active'); rp.click(); var n = 0; map.eachLayer(l => { if (l instanceof L.Polygon && l.options.dashArray === '8,5') n++; }); var skryto = n === 0 && !AGOkoli.viditelne(); rp.click(); var n2 = 0; map.eachLayer(l => { if (l instanceof L.Polygon && l.options.dashArray === '8,5') n2++; }); AGOkoli.smazPrekazku(AGOkoli.prekazky().length - 1); return { vid: vid, skryto: skryto, zpet: n2 >= 1 }; }""" % (LAT, LNG, LAT, LNG))
        ok('M7 s prekazkou se radek ukaze, prepinac ji schova a zase ukaze', m7 and m7['vid'] and m7['skryto'] and m7['zpet'], m7)
        m8 = await page.evaluate("() => { AGMapaPanel.tab('nastroje'); var ids = ['ms-t-3d', 'ms-t-kvgps', 'ms-t-nacrt', 'ms-t-oblast'].map(i => { var b = document.getElementById(i); return b && !b.hidden && b.closest('.ms-tab') && !b.closest('.ms-tab').hidden; }); var lbl = [].map.call(document.querySelectorAll('.ms-tab[data-ms-tab=\"nastroje\"] .ms-lbl'), e => e.textContent); var prek = !!document.getElementById('btn-prekazka'); return { ids: ids, lbl: lbl, prek: prek, zk: !document.getElementById('ag-zkouska-btn') }; }")
        ok('M8 zalozka Nastroje: dlazdice 3D, Kde se da merit, Nacrt, Stahnout oblast ve trech skupinach; Prekazka adoptovana; Zkouska mapy pryc', m8 and all(m8['ids']) and len(m8['lbl']) == 3 and m8['prek'] and m8['zk'], m8)
