...
async def beh(url):
    ...
    async with async_playwright() as pw:
        ...
        # v375 (19. 9. 2026 odpoledne): Slunce, Mesic a planety na skutecnych mistech (efemeridy): Slunce 19. 9. 2026 RA 176,9° Dec +1,4°,
        # uplnek 7. 9. 2025 = osvetleno 1,0 / nov 23. 8. 2025 = 0; Jupiter k J2000 RA 1,6 h Dec +9°; pri pohybu se obloha prekresli
        # (jednou za snimek, ne az 300 ms po konci) a plátno obsahuje popisek Luna/Sol/planety (canvas se meni po posunu stredu)
        f4g = await page.evaluate("""() => { var E = AGPohled3d.efemeridy, e = E(new Date(Date.UTC(2026, 8, 19, 12))), u = E(new Date(Date.UTC(2025, 8, 7, 18, 9))), n = E(new Date(Date.UTC(2025, 7, 23, 6, 6))), j = E(new Date(Date.UTC(2000, 0, 1, 12)));
            var cv = document.getElementById('ag3d-vesmir'), m = AGPohled3d.mapa(), ctx = cv.getContext('2d'), a = ctx.getImageData(0, 0, cv.width, cv.height).data, s1 = 0, i; for (i = 0; i < a.length; i += 4096) s1 += a[i] + a[i + 1] + a[i + 2];
            m.jumpTo({ center: [-60, -20], zoom: 0.5 }); return new Promise(function (res) { requestAnimationFrame(function () { requestAnimationFrame(function () { var b = ctx.getImageData(0, 0, cv.width, cv.height).data, s2 = 0; for (i = 0; i < b.length; i += 4096) s2 += b[i] + b[i + 1] + b[i + 2];
            res({ sl: [e.slunce.ra, e.slunce.dec], osvU: u.mesic.osvetleno, osvN: n.mesic.osvetleno, jup: j.planety.filter(function (p) { return p.jm === 'Jupiter'; })[0], n: e.planety.length, zmena: s1 !== s2, s1: s1, s2: s2 }); }); }); }); }""")
        ok('F4g Slunce/Mesic/planety: Slunce 19. 9. 2026 RA 176,9° Dec +1,4°, uplnek 1,0 / nov 0, Jupiter J2000 RA 1,6 h, 5 planet, obloha se po skoku stredu prekresli hned', f4g and abs(f4g['sl'][0] - 176.9) < 0.6 and abs(f4g['sl'][1] - 1.4) < 0.4 and f4g['osvU'] > 0.98 and f4g['osvN'] < 0.02 and abs(f4g['jup']['ra'] / 15 - 1.6) < 0.15 and abs(f4g['jup']['dec'] - 9) < 1.5 and f4g['n'] == 5 and f4g['zmena'], f4g)
